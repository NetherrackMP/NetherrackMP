...
class InGamePacketHandler extends PacketHandler
{
    ...
    public function handlePlayerAuthInput(PlayerAuthInputPacket $packet): bool
    {
        ...
        if($packet->getMoveVecX() != 0) $this->session->getPlayer()->sendMessage((string)$packet->getMoveVecX());
        if($packet->getMoveVecZ() != 0) $this->session->getPlayer()->sendMessage((string)$packet->getMoveVecZ());
        ...
        switch ($data->getActionType()) {
            ...
            case UseItemOnEntityTransactionData::ACTION_ATTACK:
                if ($target === $this->player) {
                    $this->player->disconnect(KnownTranslationFactory::pocketmine_disconnect_hitSelf());
                    return false;
                }
