...
class MegaPineTree extends MegaRedwoodTree{
	...
	public function generate(ChunkManager $world, Random $random, int $sourceX, int $sourceY, int $sourceZ) : bool{
		$generated = parent::generate($world, $random, $sourceX, $sourceY, $sourceZ);
		if($generated){
			$this->generatePodzol($sourceX, $sourceY, $sourceZ, $world, $random);
		}
		return $generated;
	}
	...
	private function generatePodzol(int $sourceX, int $sourceY, int $sourceZ, ChunkManager $world, Random $random) : void{
		...
		for($i = 0; $i < 5; ++$i){
			...
			if($n % 8 === 0 || $n % 8 === 7 || intdiv($n, 8) === 0 || intdiv($n, 8) === 7){
				$this->generatePodzolPatch($sourceX - 3 + $n % 8, $sourceY, $sourceZ - 3 + intdiv($n, 8), $world);
			}
		}
	}
