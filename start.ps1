...
param (
	...
	[switch]$Loop = $false,
	[string]$file = "",
	[string][Parameter(ValueFromRemainingArguments)]$extraNetherrackArgs
)
...
if($file -eq ""){
	if(Test-Path "Netherrack-MP.phar"){
	    $file = "Netherrack-MP.phar"
	}else{
...
function StartServer{
	$command = "powershell -NoProfile " + $binary + " " + $file + " " + $extraNetherrackArgs
	iex $command
}
